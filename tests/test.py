...
class TestDataPreprocessing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = pd.DataFrame({
            ...
            'country': np.random.choice(['USA', 'UK', 'France', 'Germany'], n_samples),
            'city': np.random.choice(['New York', 'London', 'Paris', 'Berlin'], n_samples),
            'property_type': np.random.choice(['Apartment', 'Villa', 'Townhouse', 'Studio'], n_samples),
            'furnishing_status': np.random.choice(['Furnished', 'Unfurnished', 'Semi-Furnished'], n_samples),
    ...
    def test_engineer_features(self):
        ...
        self.assertEqual(engineered_row['property_age'], 2025 - original_row['constructed_year'])
        self.assertEqual(engineered_row['price_per_sqft'], original_row['price'] / original_row['property_size_sqft'])
        self.assertEqual(engineered_row['total_rooms'], original_row['rooms'] + original_row['bathrooms'])
...
class TestFeatureEngineering(unittest.TestCase):
    ...
    def test_affordability_calculation(self):
        ...
        np.testing.assert_array_almost_equal(
            df_engineered['affordability_score'].values,
        ...
        np.testing.assert_array_equal(
            df_engineered['risk_score'].values,
            expected_risk
        )
...
class TestModelInputOutput(unittest.TestCase):
    ...
    def test_feature_shape_consistency(self):
        ...
        actual_feature_count = len(df_engineered.columns) - 2  # -2 for property_id and decision
