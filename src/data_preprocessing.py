...
def check_data_quality(df: pd.DataFrame) -> Dict:
    ...
    logger.info(logger.info("Data quality check completed:"))
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["disposable_income_ratio"] = (
        df["customer_salary"] - df["monthly_expenses"]
    ) / df["customer_salary"]

    logger.info(
        f"Feature engineering completed. New features created: {df.shape[1] - len(df.columns) + 10}"
    )
...
def scale_features(
    ...
    logger.info(
        logger.info(
            f"Feature scaling completed for {len(numerical_cols)} numerical features"
        )
    )
...
def preprocess_pipeline(filepath: str, random_state: int = 42) -> Tuple:
    """
