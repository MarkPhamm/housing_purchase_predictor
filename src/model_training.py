...
def train_model(
    model: Any, X_train: pd.DataFrame, y_train: pd.Series, model_name: str
) -> Any:
    """
...
def train_and_evaluate_all_models(
    ...
    for model_name, config in models_config.items():
        try:
            # Train model
            trained_model = train_model(config["model"], X_train, y_train, model_name)

            # Evaluate on validation set
            val_metrics = evaluate_model(
                trained_model, X_val, y_val, model_name, "Validation"
            )
...
def select_best_model(
    results_df: pd.DataFrame, trained_models: Dict, metric: str = "f1_score"
) -> Tuple[str, Any]:
    """
...
def get_feature_importance(
    model: Any, feature_names: list, model_name: str, top_n: int = 20
) -> pd.DataFrame:
    """
...
def save_model_and_artifacts(
    ...
    # Save model
    model_filename = (
        f"{save_dir}/best_model_{model_name.replace(' ', '_')}_{timestamp}.pkl"
    )
...
def generate_detailed_report(
    results_df: pd.DataFrame, best_model_name: str, test_metrics: Dict
) -> str:
    """
