    ...
    def preprocess_input(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in categorical_cols:
            if col in df.columns:
                ...
                if mask.any():
                    logger.warning(
                        f"Found {mask.sum()} unknown categories in '{col}'. Mapping to '{encoder.classes_[0]}'"
                    )
        ...
        for feature in self.feature_names:
            if feature not in df.columns:
                logger.warning(
                    f"Feature {feature} not found in input data, setting to 0"
                )
                df[feature] = 0
        ...
        results = pd.DataFrame(
            {
                ...
                "prediction_label": [
                    "Buy" if p == 1 else "Not Buy" for p in predictions
                ],
            }
        )
