...
def main():
    ...
    # Header
    st.markdown(
        '<div class="main-header">🏠 House Purchase Predictor</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="sub-header">Predict house purchase decisions using AI-powered machine learning</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        ...
        st.text("Model: Random Forest")
        st.text(f"Trained: {timestamp}")
        st.text("Features: 33")

    # Main content tabs
    tab1, tab2, tab3 = st.tabs(
        ["🎯 Single Prediction", "📊 Batch Prediction", "ℹ️ Information"]
    )
        ...
        with st.form("prediction_form"):
            ...
            with col1:
                ...
                previous_owners = st.number_input(
                    "Previous Owners", min_value=0, max_value=10, value=2, step=1
                )

            with col2:
                st.subheader("🏡 Property Features")
                rooms = st.number_input(
                    "Rooms", min_value=1, max_value=10, value=3, step=1
                )
                bathrooms = st.number_input(
                    "Bathrooms", min_value=1, max_value=8, value=2, step=1
                )
                garage = st.selectbox(
                    "Garage", [0, 1], format_func=lambda x: "Yes" if x == 1 else "No"
                )
                garden = st.selectbox(
                    "Garden", [0, 1], format_func=lambda x: "Yes" if x == 1 else "No"
                )
                crime_cases_reported = st.number_input(
                    "Crime Cases Reported", min_value=0, max_value=10, value=0, step=1
                )
                legal_cases_on_property = st.number_input(
                    "Legal Cases on Property", min_value=0, max_value=5, value=0, step=1
                )
                satisfaction_score = st.slider(
                    "Satisfaction Score", min_value=1, max_value=10, value=7, step=1
                )
                neighbourhood_rating = st.slider(
                    "Neighbourhood Rating", min_value=1, max_value=10, value=7, step=1
                )
                connectivity_score = st.slider(
                    "Connectivity Score", min_value=1, max_value=10, value=7, step=1
                )
            ...
            with col3:
                ...
                loan_tenure_years = st.selectbox(
                    "Loan Tenure (Years)", [10, 15, 20, 25, 30], index=4
                )
            ...
            submit_button = st.form_submit_button(
                label="🔮 Predict Purchase Decision", use_container_width=True
            )
        ...
        if submit_button:
            ...
            with st.spinner("Making prediction..."):
                try:
                    ...
                    with col1:
                        fig_not_buy = create_gauge_chart(
                            prob_not_buy, "Probability of NOT Buying"
                        )
    ...
    with tab2:
        ...
        st.write(
            "Upload a CSV file with multiple customers to get predictions for all at once."
        )
